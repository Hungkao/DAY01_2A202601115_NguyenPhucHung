...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        prompt = (result.get("prompt", "")[:40]).ljust(40)
        gpt4o_response = (result.get("gpt4o_response", "")[:40]).ljust(40)
        mini_response = (result.get("mini_response", "")[:40]).ljust(40)
        gpt4o_latency = str(result.get("gpt4o_latency", 0.0))
        mini_latency = str(result.get("mini_latency", 0.0))
        rows.append(
            f"{prompt} | {gpt4o_response} | {mini_response} | {gpt4o_latency} | {mini_latency}"
        )
